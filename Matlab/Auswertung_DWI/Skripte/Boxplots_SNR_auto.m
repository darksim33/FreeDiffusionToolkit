...
results_folder = 'C:\Users\essjud01\Desktop\Messung_single_b_030724\DICOM\00002B00\AAA0DB34\AA4B36F2_translated_as_nifti\MRT2-test_Phantom-NNLS_GAPF88804\20240703_1637\Boxplots\mit_bval';

% NIfTI-Toolbox laden
addpath('C:\Users\essjud01\Downloads\NIfTI_20140122');

% Dateien im Ergebnisordner auflisten
result_files = dir(fullfile(results_folder, '*_geometric_mean_combined.nii'));
...
for f = 1:num_files
    ...
    % Extrahiere den Basisnamen aus dem Dateinamen der NIfTI-Datei
    [~, base_name, ~] = fileparts(filename);
    
    % Korrekten Basisnamen finden (ohne "_geometric_mean_combined")
    base_name_parts = strsplit(base_name, '_');
    base_name_corrected = strjoin(base_name_parts(1:end-3), '_');
    
    % Konstruiere den Dateinamen für die .bval Datei
    b_values_file = fullfile(results_folder, [base_name_corrected '.bval']);
    
    % Konstruiere den Dateinamen für die Masken-Datei
    mask_file = fullfile(results_folder, [base_name_corrected '_mask.nii']);
    ...
    img_filename = fullfile(results_folder, [base_name_corrected '_boxplot.png']);
